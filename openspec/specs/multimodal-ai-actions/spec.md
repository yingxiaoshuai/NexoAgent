# multimodal-ai-actions Specification
...
### Requirement: Multimodal Independence From Removed Utility Tools
Multimodal capabilities SHALL remain callable after tool-surface reduction and MUST not depend on removed file, HTTP, skills, or scheduled-task tools to complete their supported workflows.

#### Scenario: Vision analysis still works after tool reduction
- **WHEN** removed utility tools are no longer present in the runtime
- **THEN** image analysis requests SHALL still complete through the preserved multimodal tool path
